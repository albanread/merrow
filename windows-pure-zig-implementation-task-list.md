...
5. Treat preview-mode diagram coverage as a parity requirement, not a follow-up.
   - Windows preview must support every Mermaid graphics type that the app already supports for PNG or SVG export.
   - Existing PNG and SVG export paths are the correctness reference for what preview mode needs to render.
...
1. Use `windows-preview-coverage-matrix.md` as the current preview-family coverage audit.
2. It distinguishes core renderer availability in `src/` from what the current studio FFI in `app/preview.zig` already exposes.
...
8. Audit preview coverage against export coverage in `app/preview.zig`.
   - Identify which diagram families are already supported by `merrow_studio_render_preview_png(...)` and `merrow_studio_export_svg(...)` but not yet by `merrow_studio_build_scene(...)`.
   - Use that gap list as the Windows preview backlog, starting with sequence, class, ER, and state if they are still missing from the direct scene path.

## Phase 2: Build and Packaging Foundation

9. Add a Windows studio target in `build.zig`.
   - Keep `merrow` CLI unchanged.
   - Add `merrow-studio` for Windows only.
   - Continue using `app/main.zig` as the shared Zig entrypoint.
10. Add `zigwin32` to `build.zig.zon`.
   - Import Win32, Direct2D, DirectWrite, common controls, file dialogs, and shell APIs through generated Zig bindings.
11. Link the required Windows libraries.
   - `user32`
   - `gdi32`
   - `comctl32`
   - `d2d1`
...
12. Create Windows packaging and run scripts.
...
13. Create `app/platform/windows_main.zig`.
   - Export `merrow_studio_main(...)` to satisfy `app/main.zig`.
   - Register the window class.
   - Create the main window.
   - Run the Win32 message loop.
14. Build the top-level window layout.
...
15. Add a native menu and toolbar equivalent.
...
16. Add DPI awareness at process startup.
...
17. Host a native multiline editor control.
...
18. Recreate the current edit loop from `app/platform/macos_app.m`.
...
19. Implement the command box workflow.
...
20. Implement diagram shuffle and direction actions.
...
21. Implement a custom preview window class backed by Direct2D.
   - Use `merrow_studio_build_scene(...)` where a direct scene path exists.
   - Draw subgraphs, nodes, edges, and edge labels from the exported scene structs.
22. Expand preview coverage to all supported graphics types.
   - Preview mode must work for every diagram family already supported by PNG or SVG export.
   - Do not stop at flowcharts if sequence, class, ER, state, journey, gantt, pie, or other supported graphics types already have export coverage.
   - Track the missing direct-scene families and either add scene builders for them or route preview through a fallback renderer until direct scene support exists.
23. Treat PNG and SVG export output as the preview rendering reference.
   - Use the existing PNG and SVG renderers to validate geometry, colors, label placement, and supported diagram-family coverage.
   - When direct preview rendering disagrees with export output, treat export output as the current source of truth unless there is a deliberate product reason to differ.
24. Add DirectWrite text rendering.
...
25. Add pan and zoom behavior.
   - Mouse wheel zoom.
   - drag pan.
   - Fit and reset actions.
26. Keep PNG and SVG-based preview fallbacks available.
   - If direct scene rendering fails or does not yet exist for a diagram type, preserve the ability to load a rendered preview image from `merrow_studio_render_preview_png(...)`.
   - If SVG export is the stronger renderer for a given family, allow the Windows preview backlog to use that path as an intermediate correctness reference as well.
   - The fallback path is acceptable as a starting point, but the Windows app must still present a working preview for all supported graphics types.

## Phase 6: Freeform Canvas Mode

27. Create a Windows equivalent of the freeform canvas component.
...
28. Reimplement canvas drawing in Zig.
...
29. Reimplement canvas interactions.
   - Click selection.
   - drag move.
...
32. Implement Mermaid document open and save.
   - `.mmd` source files.
   - dirty-state tracking.
   - recent path handling if needed later.
33. Decide and document the freeform document format.
...
34. Implement file open and save flows for both modes.
   - Mermaid source mode.
   - freeform document mode.
35. Implement export dialogs and export actions.
...
36. Add Windows smoke tests for the shared Zig layer.
   - syntax check
   - build scene
   - build editable graph
   - export PNG
   - export SVG
37. Create a manual parity checklist against the mac app.
   - source editing
   - live preview updates
   - preview coverage across every supported diagram family
   - command application
   - shuffle
   - mode switching
   ...
   - save and reload
   - export
38. Build a Windows visual-check workflow.
   - Render the `test-diagrams/` set from the studio-backed path where possible.
   - Include every supported diagram family in the preview validation set, not only flowcharts.
   - Capture screenshots for key interactive flows.
39. Add packaging verification.
...
2. Milestone B: Preview viewport can show every supported diagram family, using direct scene rendering where available and PNG or SVG fallback where needed.
3. Milestone C: Preview viewport has pan and zoom and begins replacing fallback paths with direct scene rendering family by family.
...
5. Make preview mode work for every supported diagram family, using existing PNG and SVG export paths as the reference and fallback while direct preview renderers are filled in.